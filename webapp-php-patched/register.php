    ...
    if (!$username || !$email || !$password) {
        ...
        try {
            ...
            if ($stmt->num_rows > 0) {
                ...
                $stmt->close();
                $role   = 'user'; // LUON la 'user' - KHONG bao gio lay role tu input cua client
                $salary = 0;
                ...
                if ($stmt2->execute()) {
                    // Tao JWT (HS256) ngay sau khi dang ky thanh cong.
                    // $role o day la bien server tu gan ('user'), KHONG phai gia tri tu $_POST,
                    // nen token tao ra luon dung voi quyen thuc te trong DB.
                    require_once 'jwt.php';
                    $token = jwt_create($username, $role);
